...
public class DateTimeUtil{
	...
	public static SimpleDateFormat getDateCPVTask() {
		return new SimpleDateFormat("yyyyMMdd");
	}
	public static SimpleDateFormat getDatetimeCPVTask() {
	...
	public static int getLastDateOfYearMonth(int year, int month){
		Calendar batdau=Calendar.getInstance();
		batdau.set(year, month-1, 1, 0, 0, 0);
		return batdau.getActualMaximum(Calendar.DATE);
	}

	public static Date getDateStartOfYearMonth(int year, int month){
		Calendar batdau=Calendar.getInstance();
		batdau.set(year, month-1, 1, 0, 0, 0);
		return batdau.getTime();
	}

	public static Date getDateEndOfYearMonth(int year, int month, int day){
		Calendar batdau=Calendar.getInstance();
		batdau.set(year, month-1, day, 0, 0, 0);
		return batdau.getTime();
	}
	...
	public static Date nextDate(Date date, int number){
		Calendar customDate=Calendar.getInstance();
		customDate.setTime(date);
		customDate.add(Calendar.DATE, number);
		return customDate.getTime();
	...
	public static long currentTimeMillisFuture() {
		return System.currentTimeMillis()+(60000*1);
	}
	...
	public static Date nextMinutes(Date date, int number){
		Calendar customDate=Calendar.getInstance();
		customDate.setTime(date);
		customDate.add(Calendar.MINUTE, number);
		return customDate.getTime();
	}

	// Ngưỡi để phân biệt giữa seconds và milliseconds (ví dụ: thời gian từ epoch đến năm 2000)
	private static final long THRESHOLD = 946684800000L; // Milliseconds từ epoch đến 2000-01-01
	...
	public static boolean isDateBetweenTwoDate(Date targetDate,Date dateFirst,Date dateSeccond) {
		
		LocalDateTime localDateTimeTarget = targetDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
		LocalDateTime localDateTimeFirst = dateFirst.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
		LocalDateTime localDateTimeSeccond = dateSeccond.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
		
		return (localDateTimeTarget.isAfter(localDateTimeFirst) || localDateTimeTarget.isEqual(localDateTimeFirst)) 
	               && (localDateTimeTarget.isBefore(localDateTimeSeccond) || localDateTimeFirst.isEqual(localDateTimeSeccond));
	}
}
