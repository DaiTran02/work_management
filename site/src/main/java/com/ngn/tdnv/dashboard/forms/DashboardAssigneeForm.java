...
public class DashboardAssigneeForm extends VerticalLayoutTemplate implements FormInterface{
	...
	public void createLayout() {
		vLayout.removeAll();
		H1 header = new H1("THỐNG KÊ NHIỆM VỤ ĐƯỢC GIAO");
		header.getStyle()
		...
		H1 countTask = new H1(countMenuUtil.countTaskAssignee()+" ");
		...
		vLayout.add(header,hLayoutTitle,taskOverviewSummaryFrom,hlayoutChartAndAchiment,bsChartSummary);
		createLayoutEvent();
	...
	private void createLayoutEvent() {
		try {
			ApiResultResponse<List<ApiTaskSummaryModel>> dataSummary = ApiTaskService.getSummaryTaskAssignee(getSearch());
			if(dataSummary.isSuccess()) {
				...
				stringNavigate.append("task?status=");
				buildEventButton();
			}
		} catch (Exception e) {
			e.printStackTrace();
...
//				+ "                            'Nhiệm vụ<br/>' +"
//				+ "                            '<strong>"+countMenuUtil.countTaskAssignee()+"</strong>'"
//				+ "                        )"
//				+ "                            .css({"
//				+ "                                color: '#000',"
//				+ "                                textAnchor: 'middle'"
...
//	}
	...
	//Summary chart
	private int idChart = 100;
	private void loadChartSummary(List<ApiTaskSummaryModel> listData) {
		bsChartSummary.removeAll();
		bsChartSummary.setWidthFull();
		bsChartSummary.getStyle().setPaddingLeft("0").setPadding("0");
		BsRow row = bsChartSummary.addRow();
	...
	private Component createLayoutCircleChartAchivement(String id,String name,String data) {
		...
		Html html = new Html("<script>"
				...
				+ "            '{point.name}</b><br/>' +"
				+ "            '"+name+": <b>{point.y}</b><br/>' +"
				+ "            'Tổng nhiệm vụ: <b>"+countMenuUtil.countTaskAssignee()+"</b><br/>'"
				+ "    },"
				+ "    series: [{"
				+ "        minPointSize: 10,"
				+ "        innerSize: '20%',"
	...
	private ApiFilterTaskModel getSearch() {
		...
		if(isUser) {
			...
			apiFilterTaskModel.setAssigneeOrganizationUserId(user.getId());
		}
	...
	private List<ApiNameAndValueModel> getDataAchiment(){
		...
		apiFilterAchivementModel.setAssigneeOrganizationId(belongOrganization.getOrganizationId());
		if(isUser) {
			apiFilterAchivementModel.setAssigneeOrganizationId(null);
			apiFilterAchivementModel.setAssigneeOrganizationUserId(user.getId());
		}

		ApiResultResponse<List<ApiNameAndValueModel>> listAchivement = ApiTaskService.getListAchivementTaskAssignee(apiFilterAchivementModel);
