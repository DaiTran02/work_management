...
public class DashboardFollowerForm extends VerticalLayoutTemplate implements FormInterface{
	...
	private HorizontalLayout hLayoutChartAndAchivement = new HorizontalLayout();
	private BsLayout bsLayoutSummary = new BsLayout();

	public DashboardFollowerForm() {
		loadCmbTypeOfChart();
		buildLayout();
		configComponent();
	}
	...
	public void createLayout() {
		...
		H1 header = new H1("THỐNG KÊ NHIỆM VỤ ĐƠN VỊ ĐANG THEO DÕI");
		header.getStyle()
		...
		H1 countTask = new H1(countMenuUtil.countTaskFollow()+" ");
		countTask.getStyle().setColor("black");
		...
		hLayoutChartAndAchivement.setWidthFull();
		hLayoutChartAndAchivement.getStyle().set("gap", "25px");
		if(isMobileLayout) {
			hLayoutChartAndAchivement.getStyle().setDisplay(Display.FLEX).setFlexDirection(FlexDirection.COLUMN).set("gap", "0");
		}

		loadDataStatus();
		vLayout.add(header,hLayoutTitle,taskOverviewSummaryFrom,hLayoutChartAndAchivement,bsLayoutSummary);
		loadLayoutStatusAndAchivement();
		createLayoutEvent();
	}
	
	private void loadLayoutStatusAndAchivement() {
		loadDataStatus();
		hLayoutChartAndAchivement.removeAll();
		...
		if(cmbTypeOfChart.getValue().getKey().equals("collumn")) {
			hLayoutChartAndAchivement.add(createChartKpiLayout(id),createLayoutCollumnChartAchivement(idAchiment, "Thành tích", dataAchivement));
		}else {
			hLayoutChartAndAchivement.add(createChartCircleKpiLayout(id,totalChartKpi),createLayoutCircleChartAchivement(idAchiment, "Thành tích", dataCircleAchiment));
		}
	}
	...
	private void createLayoutEvent() {
		...
		try {
			...
			if(dataSummary.isSuccess()) {
				...
				stringNavigate.append("task_follower?status=");
				buildEventButton();
...
//				+ "                            'Nhiệm vụ<br/>' +"
//				+ "                            '<strong>"+countMenuUtil.countTaskFollow()+"</strong>'"
//				+ "                        )"
//				+ "                            .css({"
//				+ "                                color: '#000',"
//				+ "                                textAnchor: 'middle'"
...
//	}
	...
	//Summary chart
	private int idChart = 300;
	private void loadChartSummary(List<ApiTaskSummaryModel> listData) {
		bsLayoutSummary.removeAll();
		bsLayoutSummary.setWidthFull();
		bsLayoutSummary.getStyle().setPaddingLeft("0").setPaddingRight("0");
		
		BsRow row = bsLayoutSummary.addRow();
...
//	}
	...
	private ApiFilterTaskModel getSearch() {
		...
		apiFilterTaskModel.setFollowerOrganizationId(belongOrganization.getOrganizationId());
		if(isUser) {
			apiFilterTaskModel.setOwnerOrganizationId(null);
			apiFilterTaskModel.setFollowerOrganizationId(null);
			apiFilterTaskModel.setFollowerOrganizationUserId(user.getId());
		}
	...
	private Component createLayoutCircleChartAchivement(String id,String name,String data) {
		...
		Html html = new Html("<script>"
				...
				+ "            '{point.name}</b><br/>' +"
				+ "            '"+name+": <b>{point.y}</b><br/>' +"
				+ "            'Tổng nhiệm vụ: <b>"+countMenuUtil.countTaskFollow()+"</b><br/>'"
				+ "    },"
				+ "    series: [{"
				+ "        minPointSize: 10,"
				+ "        innerSize: '20%',"
	...
	private List<ApiNameAndValueModel> getDataAchiment(){
		...
		apiFilterAchivementModel.setFollowerOrganizationGroupId(signInOrgModel.getGroup().getId());
		if(isUser) {
			apiFilterAchivementModel.setOwnerOrganizationId(null);
			apiFilterAchivementModel.setFollowerOrganizationGroupId(null);
			apiFilterAchivementModel.setFollowerOrganizationUserId(user.getId());
		}

		ApiResultResponse<List<ApiNameAndValueModel>> listAchivement = ApiTaskService.getListAchivementTaskFollower(getSearch());
	...
	private Component createChartKpiLayout(String id) {
		Div div = new Div();
		div.setId(id);
		...
		Html html = new Html("<script>"
				...
				+ "    },"
				+ "    title: { "
				+ "        text: 'Nhiệm vụ KPI đang theo dõi' "
				+ "    }, "
				+ "    accessibility: { "
				+ "        announceNewData: { "
				+ "            enabled: true "
