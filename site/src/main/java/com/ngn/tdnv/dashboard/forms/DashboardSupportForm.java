...
public class DashboardSupportForm extends VerticalLayoutTemplate implements FormInterface{
	...
	public void createLayout() {
		vLayout.removeAll();
		H1 header = new H1("THỐNG KÊ NHIỆM VỤ PHỐI HỢP");
		header.getStyle()
		...
		H1 countTask = new H1(countMenuUtil.countTaskSupport()+" ");
	...
	private void createLayoutEvent() {
		try {
			...
			if(dataSummary.isSuccess()) {
				...
				stringNavigate.append("task_support?status=");
				buildEventButton();
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	VerticalLayout vLayoutChart = new VerticalLayout();
...
//				+ "                            'Nhiệm vụ<br/>' +"
//				+ "                            '<strong>"+countMenuUtil.countTaskSupport()+"</strong>'"
//				+ "                        )"
//				+ "                            .css({"
//				+ "                                color: '#000',"
//				+ "                                textAnchor: 'middle'"
...
//	}
	...
	private Component createLayoutCircleChartAchivement(String id,String name,String data) {
		...
		Html html = new Html("<script>"
				...
				+ "            '{point.name}</b><br/>' +"
				+ "            '"+name+": <b>{point.y}</b><br/>' +"
				+ "            'Tổng nhiệm vụ: <b>"+countMenuUtil.countTaskSupport()+"</b><br/>'"
				+ "    },"
				+ "    series: [{"
				+ "        minPointSize: 10,"
				+ "        innerSize: '20%',"
	...
	private ApiFilterTaskModel getSearch() {
		...
		if(isUser) {
			...
			apiFilterTaskModel.setSupportOrganizationUserId(user.getId());
		}
	...
	private List<ApiNameAndValueModel> getDataAchiment(){
		...
		apiFilterAchivementModel.setSupportOrganizationId(belongOrganization.getOrganizationId());
		if(isUser) {
			apiFilterAchivementModel.setSupportOrganizationId(null);
			apiFilterAchivementModel.setSupportOrganizationUserId(user.getId());
		}

		ApiResultResponse<List<ApiNameAndValueModel>> listAchivement = ApiTaskService.getListAchivementTaskSupport(apiFilterAchivementModel);
