...
public class TaskViewRedoHistoryForm extends VerticalLayoutTemplate implements FormInterface{
	private static final long serialVersionUID = 1L;
	
	private VerticalLayout vLayout = new VerticalLayout();
	private int count = 1;
	
	private List<ApiTaskRedoModel> listTaskRedo;
	public TaskViewRedoHistoryForm(List<ApiTaskRedoModel> listTaskRedo) {
		this.listTaskRedo = listTaskRedo;
		buildLayout();
		configComponent();
		loadData();
	}
	...
	private void loadData() {
		vLayout.removeAll();
		listTaskRedo.forEach(model->{
			vLayout.add(createItem(model));
		});
	}
	
	private Component createItem(ApiTaskRedoModel apiTaskRedoModel) {
		
		VerticalLayout vLayoutItem = new VerticalLayout();
		
		H4 spTitle = new H4("*NHIỆM VỤ ĐƯỢC ĐƠN VỊ GIAO YÊU CẦU THỰC HIỆN LẠI LẦN "+count+".");
		...
		vLayoutItem.add(spTitle,createLayoutKeyValue("Đơn vị yêu cầu: ", apiTaskRedoModel.getCreator().getOrganizationName(), null),
		...
		vLayoutItem.getStyle().setBackground("#edf4ff").setPadding("5px").setBorderRadius("10px").setBoxShadow("rgba(0, 0, 0, 0.16) 0px 1px 4px");
		count++;
