...
public class TaskChooseAllOrgFormv2 extends VerticalLayoutTemplate implements FormInterface{
	...
	private List<OrganizationModel> listOrgIsRemoves = new ArrayList<OrganizationModel>();
	...
	public TaskChooseAllOrgFormv2(String idParent,OrganizationModel ownerOrg,Map<OrganizationModel, UserOranizationExModel> mapOrgAssigneeWasChoose,
	...
	private void loadCurrentOrg() {
		currentOrg = getInfoOrg(idParent);
		openUserOfCurrentOrg();
	}
	
	
	private void openUserOfCurrentOrg() {
		if(currentOrg != null) {
			openFormUser(currentOrg, currentOrg.getUserOrganizationExpands());
		}else {
			taskChooseUserForOrgForm.setVisible(false);
		}
	}
	...
	private void setFolderLayout(Pair<String, String> item) {
		if(item.getLeft() == null) {
			hFolderLayout.removeAll();
			titleOrg.setText("Danh sách tổ chức");
			hFolderLayout.add(titleOrg);
...
//				});
	...
	private Component createToolbar() {
		...
		Span spNote = new Span("*Nếu chọn nhiều đơn vị thực hiện thì không thể chọn đơn vị phối hợp "
				+ "(nhấp vào tên đơn vị để xem các đơn vị trực thuộc, mặc định giao diện sẽ hiện thị các đơn vị trực thuộc của đơn vị hiện tại)");
	...
	private Component createGrid() {
		...
		grid.addComponentColumn(model->{
			...
			String checkIdParent = idParent == null ? "nah" : idParent;
			...
			if(!mapOrgAssigneeWasChoose.isEmpty() && checkSupport == false) {
				mapOrgAssigneeWasChoose.forEach((k,v)->{
					...
					if(k.getId().equals(checkIdParent)) {
						if(mapOrgAssigneeWasChoose.get(k) != null ) {
			...
			if(mapOrgSupportWasChoose != null && !mapOrgSupportWasChoose.isEmpty()) {
				...
				mapOrgIsChoose.putAll(mapOrgSupportWasChoose);

			}

			if(!mapUserOfOrg.isEmpty()) {
				mapUserOfOrg.forEach((k,v)->{
					if(model.getId().equals(k.getId())) {
						cbChooseOrg.setValue(true);
						cbChooseOrg.setReadOnly(true);
						mapOrgIsChoose.put(k, v);
					}
					
					if(checkIdParent.equals(k.getId())) {
						mapOrgIsChoose.put(k, v);
					}
				});
		...
		grid.addComponentColumn(model->{
			...
			String nameOrg = model.getId().equals(ownerOrg.getId()) == false ? model.getName() : model.getName() + " (đơn vị đang giao nhiệm vụ)";

			ButtonTemplate btnHeader = new ButtonTemplate(nameOrg);
			btnHeader.addThemeVariants(ButtonVariant.LUMO_TERTIARY);

			btnHeader.addClickListener(e->{

				Pair<String, String> item = Pair.of(model.getId(),nameOrg);
...
//					}
					...
					mapUserOfOrg.forEach((k,v)->{
						if(k.getId().equals(model.getId())) {
							...
							mapOrgAssigneeWasChoose.replace(k, null);
							if(mapOrgSupportWasChoose != null) {
	...
	private MenuBarTemplate menuBarShowListOrgIsChoose = new MenuBarTemplate();
	private void showOrgSelect() {
		...
		ButtonTemplate btnShowListOrg = new ButtonTemplate(FontAwesome.Solid.LIST_DOTS.create());
		btnShowListOrg.addThemeVariants(ButtonVariant.LUMO_TERTIARY);
		btnShowListOrg.getStyle().setColor("white");
		btnShowListOrg.addClickListener(e->{
			listOrgIsRemoves.clear();
		});

		menuBarShowListOrgIsChoose.removeAll();
		menuBarShowListOrgIsChoose.addThemeVariants(MenuBarVariant.LUMO_SMALL);
		MenuItem menuItem = menuBarShowListOrgIsChoose.addItem(btnShowListOrg);
		SubMenu subMenu = menuItem.getSubMenu();
		subMenu.add(createGridIsChoose());
		btnShowListOrg.addClickListener(e->{
			loadDataForOrgIsChoose();
		});
		...
		btnRestoreLayout.setTooltipText("Danh sách đơn vị đã chọn");
		...
		btnRestoreLayout.addClickListener(e->{
			hLayoutSelected.removeAll();
			hLayoutSelected.setWidth("350px");
			...
			isMinizime = false;
			hLayoutSelected.add(spOrg,btnSave,btnRemoveAll,menuBarShowListOrgIsChoose,btnMinimize);
		});
	...
	private Grid<OrganizationModel> gridOrgIsChoose = new Grid<OrganizationModel>(OrganizationModel.class,false);

	private Component createGridIsChoose() {
		VerticalLayout vLayoutGridIsChoose = new VerticalLayout();

		Span sp = new Span("Danh sách đơn vị đã chọn");
		sp.getStyle().setFontWeight(600);

		vLayoutGridIsChoose.add(sp);

		gridOrgIsChoose = new Grid<OrganizationModel>(OrganizationModel.class,false);

		gridOrgIsChoose.addColumn(OrganizationModel::getName).setHeader("Tên");
		gridOrgIsChoose.addComponentColumn(model->{
			ButtonTemplate btnRemove = new ButtonTemplate(FontAwesome.Solid.CLOSE.create());
			btnRemove.addThemeVariants(ButtonVariant.LUMO_TERTIARY);
			btnRemove.addThemeVariants(ButtonVariant.LUMO_ERROR);
			btnRemove.addClickListener(e->{
				listOrgIsRemoves.add(model);
				loadDataForOrgIsChoose();
			});

			return btnRemove;
		}).setWidth("80px").setHeader("Thao tác").setFlexGrow(0);

		ButtonTemplate btnConfirm = new ButtonTemplate("Xác nhận bỏ chọn");
		btnConfirm.addThemeVariants(ButtonVariant.LUMO_ERROR);
		btnConfirm.getStyle().setMarginLeft("auto");
		btnConfirm.addClickListener(e->{
			doRemoveListOrgIsChoose();
			showOrgSelect();
		});

		vLayoutGridIsChoose.setWidth("400px");
		vLayoutGridIsChoose.setHeight("600px");
		vLayoutGridIsChoose.add(gridOrgIsChoose,btnConfirm);

		return vLayoutGridIsChoose;
	}

	private OrganizationModel getInfoOrg(String id) {
		ApiResultResponse<ApiOrganizationModel> data = ApiOrganizationService.getOneOrg(id);
		if(data.isSuccess()) {
			return new OrganizationModel(data.getResult());
		}
		return null;
	}

	private void loadDataForOrgIsChoose() {
		List<OrganizationModel> listOrg = new ArrayList<OrganizationModel>();
		mapOrgIsChoose.forEach((k,v)->{
			listOrg.add(k);
		});

		listOrg.removeAll(listOrgIsRemoves);

		gridOrgIsChoose.setItems(listOrg);
	}

	private void doRemoveListOrgIsChoose() {
		listOrgIsRemoves.forEach(model->{
			for(Entry<OrganizationModel, UserOranizationExModel> m : mapOrgIsChoose.entrySet()) {
				if(m.getKey().getId().equals(model.getId())) {
					mapOrgIsChoose.remove(m.getKey());
				}
			}
			for(Entry<OrganizationModel, UserOranizationExModel> mUser : mapUserOfOrg.entrySet()) {
				if(mUser.getKey().getId().equals(model.getId())) {
					mapUserOfOrg.remove(mUser.getKey());
				}
			}
		});
		listOrgIsRemoves.clear();
