...
public class ReportFilterTaskSupportForm extends VerticalLayoutTemplate implements FormInterface{
	...
	private ComboBox<Pair<String, String>> cmbUserDoSupport = new ComboBox<Pair<String,String>>("Người thực hiện hỗ trợ");
	...
	public ReportFilterTaskSupportForm() {
		checkUiMobile();
		buildLayout();
		configComponent();
		loadData();
	...
	public void configComponent() {
		...
		cmbUserDoSupport.addValueChangeListener(e->{
			fireEvent(new ClickEvent(this,false));
		});
	...
	private void createLayout() {
		...
		cmbUserDoSupport.addThemeVariants(ComboBoxVariant.LUMO_SMALL);
		...
		hLayout.add(startDate,icon,endDate,cmbPriority,cmbStatus,
				cmbAssigneeOrgId,cmbUserDoSupport,hLayoutSearch,btnCreateReport,btnDownload,btnDownload.getAnchor(),spDesr);
	...
	private void loadData() {
		...
		loadCmbStatus();
		loadCmdAssignee();
		loadCmbUserDoSupport();
	}

	private void loadCmbUserDoSupport() {
		...
		cmbUserDoSupport.setItems(listData);
		cmbUserDoSupport.setItemLabelGenerator(Pair::getValue);
		cmbUserDoSupport.setValue(listData.get(0));
		if(isUser) {
			try {
				cmbUserDoSupport.setValue(listData.stream().filter(data-> data.getKey() == null 
						? "nah".equals(userAuthenticationModel.getId()) : data.getKey().equals(userAuthenticationModel.getId())).findFirst().get());
				cmbUserDoSupport.setReadOnly(true);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
...
//	}
	...
	public ApiFilterReportTaskModel getSearch() {
		...
		apiFilterReportTaskModel.setSupportOrganizationId(belongOrganizationModel.getOrganizationId());
		apiFilterReportTaskModel.setSupportOrganizationUserId(cmbUserDoSupport.getValue().getKey());
	...
	public ComboBox<Pair<String, String>> getCmbUserDoSupport() {
		return cmbUserDoSupport;
	}
}
