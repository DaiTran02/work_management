...
public class ReportFilterTasksOwnerForm extends VerticalLayoutTemplate implements FormInterface{
	...
	private ComboBox<Pair<String, String>> cmbOwnerAssign = new ComboBox<Pair<String,String>>("Đơn vị giao");
	...
	private ComboBox<Pair<String, String>> cmbFollowerOrgId = new ComboBox<Pair<String,String>>("Đơn vị theo dõi");
	...
	public ReportFilterTasksOwnerForm() {
		checkUiMobile();
		buildLayout();
		configComponent();
		loadData();
	...
	public void configComponent() {
		...
		cmbOwnerAssign.addValueChangeListener(e->{
			fireEvent(new ClickEvent(this,false));
		});
	...
	private void createLayout() {
		...
		cmbOwnerAssign.addThemeVariants(ComboBoxVariant.LUMO_SMALL);
		cmbOwnerAssign.setWidth("390px");
		
		
		cmbAssigneeOrgId.addThemeVariants(ComboBoxVariant.LUMO_SMALL);
		cmbAssigneeOrgId.setWidth("300px");
		
		cmbSupportOrgId.addThemeVariants(ComboBoxVariant.LUMO_SMALL);
		cmbSupportOrgId.setWidth("300px");
		
		cmbFollowerOrgId.addThemeVariants(ComboBoxVariant.LUMO_SMALL);
		cmbFollowerOrgId.setWidth("300px");
		...
		hLayout.add(startDate,icon,endDate,cmbLeaderApproveTaskId,cmbClassifyTaskId,cmbPriority,cmbStatus,cmbOwnerAssign,cmbAssigneeOrgId,cmbSupportOrgId,cmbFollowerOrgId,hLayoutSearch,
				btnCreateReport,btnDownload,btnDownload.getAnchor(),spDesr);
		...
		if(isUser) {
			cmbOwnerAssign.setVisible(false);
		}
	...
	private void loadCmbOwner() {
		...
		if(isUser) {
			listPairOwner.add(Pair.of(null,""));
		}
		...
		cmbOwnerAssign.setItems(listPairOwner);
		cmbOwnerAssign.setItemLabelGenerator(Pair::getRight);
		cmbOwnerAssign.setValue(listPairOwner.get(0));
	}
	...
	public ApiFilterReportTaskModel getSearch() {
		...
		if(cmbOwnerAssign.getValue().getKey() != null) {
			if(viewParentOrg != null && getCurrentOrg.getLevel().getKey().equals("room")) {
				if(cmbOwnerAssign.getValue() != null && cmbOwnerAssign.getValue().getKey() != null) {
					if(cmbOwnerAssign.getValue().getKey().equals("incChildOrgs") || cmbOwnerAssign.getValue().getKey().equals("incChildOrgsAndExcMyOrg")) {
						apiFilterReportTaskModel.setOwnerOrganizationId(viewParentOrg.getOrganizationId());
						apiFilterReportTaskModel.setDataScopeType(cmbOwnerAssign.getValue().getKey());
					}else {
						apiFilterReportTaskModel.setOwnerOrganizationId(cmbOwnerAssign.getValue().getKey());
					}
				}
			}else {
				if(cmbOwnerAssign.getValue().getKey().equals("incChildOrgs") || cmbOwnerAssign.getValue().getKey().equals("incChildOrgsAndExcMyOrg")) {
		...
		if(isUser) {
			apiFilterReportTaskModel.setOwnerOrganizationId(null);
			apiFilterReportTaskModel.setOwnerOrganizationUserId(userAuthenticationModel.getId());
		}
...
//		apiFilterReportTaskModel.setFollowerOrganizationGroupId(cmbFollowerOrgId.getValue().getKey());
	...
	public ComboBox<Pair<String, String>> getCmbOwnerAssign() {
		return cmbOwnerAssign;
	}
}
